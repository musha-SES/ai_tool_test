...
-   `processSelfEvaluationSheets()`
    -   **役割**: Googleドライブフォルダ内の自己評価シート（CSVファイル）を読み込み、匿名化してGeminiに分析させ、結果をChatworkで通知する。
    -   **処理フロー**:
        1.  `getSelfEvaluationFolderId()` で自己評価シートが格納されているフォルダIDを取得する。
        2.  `getSelfEvaluationNameColumnHeader()` で氏名列のヘッダー名を取得する。
        3.  指定されたフォルダからファイル名に「自己評価」を含むCSVファイルを検索し、一つずつ処理する。
        4.  各CSVファイルの内容を `parseCsvData()` でパースする。
        5.  パースしたデータから氏名列を特定し、個人を特定できないように匿名化する。
...
-   `parseCsvData(csvText)`
    -   **役割**: CSV形式のテキストデータをパースし、2次元配列として返す。ダブルクォートで囲まれたフィールド内のカンマや改行も正しく処理する。
