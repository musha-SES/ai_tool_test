const CONFIG = {
  ...
  INDIVIDUAL_REPORT_PROMPT_TEMPLATE: `以下の{anonymousName}の過去1週間分の日報データから、このメンバーのコンディションの傾向、個人的な課題、ポジティブな動きについて分析し、簡潔なレポートを生成してください。マネージャーが週ごとの傾向を**一目で把握できるよう、以下のフォーマットに厳密に従って**記述してください。

**過去1週間分の{anonymousName}の日報データ：**
{reportEntries}
